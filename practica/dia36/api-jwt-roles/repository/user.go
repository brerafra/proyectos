...
func (r *sqlUserRepository) Create(u *domain.User) error {
	q := `INSERT INTO users3(name, card, email, password, is_active, is_admin, permissions) VALUES($1, $2, $3, $4, $5, $6, $7);`
	if _, err := r.db.Exec(r.ctx, q, u.Name, u.Card, u.Email, u.Password, u.IsActive, u.IsAdmin, u.Permissions); err != nil {
		return err
	}
	return nil
}
...
func (r *sqlUserRepository) GetById(id int64) (*domain.User, error) {
	q := `SELECT user_id, name, card, email, password, is_active, is_admin, permissions FROM users3 WHERE user_id=$1`
...
func (r *sqlUserRepository) GetUserByEmail(email string) (*domain.User, error) {
	q := `SELECT user_id, name, card, email, password, is_active, is_admin, permissions FROM users3 WHERE email=$1`
...
func (r *sqlUserRepository) GetAll(page, limit int) ([]domain.User, int, error) {
	...
	q := `SELECT user_id, name, card, email, password, is_active, is_admin, permissions FROM users3 LIMIT $1 OFFSET $2`
...
func (r *sqlUserRepository) Update(s *domain.User) error {
	q := `UPDATE users3 set name=$1, card=$2, email=$3, password=$4, is_active=$5, is_admin=$6, permissions=$7 WHERE user_id=$8`
	if _, err := r.db.Exec(r.ctx, q, s.Name, s.Card, s.Email, s.Password, s.IsActive, s.IsAdmin, s.Permissions, s.UserId); err != nil {
		return err
	}
	return nil
}
...
func (r *sqlUserRepository) Delete(id int64) error {
	q := `DELETE FROM users3 WHERE user_id=$1`
